...
Hacemos la union de los datasets de los estudiantes de la clase de portuges y matem?ticas

```{r}
d3 <- rbind(d1, d2)
```

Segun el archivo de Kaggle, 382 estudiantes se repiten en los dos datasets. En tanto debemos detectarlos y solo 
...
### cuales elemntos no se repiten. Ahora, filas.unicas contendra esta informacion.
```{r}
...
### Tambi?n, se usar? una ecuaci?n que nos permite encontrar cu?l ser?a el consumo de alcohol en la totalidad que dar?a sumar Walc y Dalc. 

Se le denomina Alc y consiste en multiplicar los d?as que comprender?a Walc y Dalc de la semana sobre el total de estos. Determinando asi un valor que es un espectro m?s amplio de ?mbos. Se utiliza la funci?n round() con par?metro digits=0 para obtener numeros redondeados del 1-5 de nuevo.
